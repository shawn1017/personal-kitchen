...
export default function RecipeDetailPage() {
  ...
  return (
    <View className='pk-page detail-page'>
      {recipe.coverImage ? <Image className='detail-cover' src={recipe.coverImage} mode='aspectFill' /> : <View className='detail-cover pk-cover-fallback'>{recipe.name.slice(0, 1)}</View>}
      ...
      <View className='detail-section pk-card'><View className='detail-section-title'>做法步骤</View>{recipe.steps.length ? recipe.steps.map((step, index) => <View key={step.id} className='detail-step'><View className='step-number'>{index + 1}</View><View className='step-content'>{step.image && <Image className='step-image' src={step.image} mode='aspectFill' />}<View>{step.text || '待补充说明'}</View>{step.timerSeconds && <View className='step-timer'>计时 {Math.round(step.timerSeconds / 60)} 分钟</View>}</View></View>) : <View className='section-empty'>暂未填写步骤</View>}</View>
